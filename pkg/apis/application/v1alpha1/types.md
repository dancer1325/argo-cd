...
* == resource filter + list[jsonPaths] / 
  * | compare vs live state, ignored 
...
* ``ManagedFieldsManagers []string `json:"managedFieldsManagers,omitempty" protobuf:"bytes,7,opt,name=managedFieldsManagers"``
  * == list of trusted managers /
    * (fields mutated by those managers) priority > (desired state | SCM) priority
...
* == map BETWEEN CRD's field -- & -- Kubernetes' native-type
* uses
  * resources unit conversion / ❌type is NOT explicitly defined ❌
    * _Example:_ CPU requests
      * convert "0.1" -- to -- "100m"  
* ``Field string `json:"field,omitempty" protobuf:"bytes,1,opt,name=field"``
  * == JSON path -- to the -- specific CRD's field / requires type conversion
  * _Example:_ "spec.resources.requests.cpu"
* ``Type string `json:"type,omitempty" protobuf:"bytes,2,opt,name=type"``
  * == `Field`'s expected Kubernetes native-type
}
...
* holds configuration -- to -- customize 
  * resource diffing
  * health assessment
...
* ``KnownTypeFields []KnownTypeField `protobuf:"bytes,4,opt,name=knownTypeFields"``
  * == `[]KnownTypeField` | unit conversions should be applied
}
