...
TODO: Argo CD provides a mechanism to override the parameters of Argo CD applications that leverages config management
tools
* provides
  * flexibility in having most of the application manifests defined in Git, while leaving room
  for *some* parts of the  k8s manifests determined dynamically, or outside of Git
* It also serves as an alternative way of
...
> truth becomes a union of the Git repository, and the application overrides
* The Argo CD parameter
> overrides feature is provided mainly as a convenience to developers and is intended to be used in
> dev/test environments, vs
* production environments.

* `argocd app set -p (COMPONENT=)PARAM=VALUE`
  * `PARAM`
    * == normal YAML path
  * way to use parameter overrides
...
* [`argocd app set`'s tool-specific flags](./commands/argocd_app_set.md)
  * _Examples:_ `--kustomize-image`, `--jsonnet-ext-var-str`, etc.
* TODO: You can also specify overrides directly in the source field on the application spec
* [supported options](./application_sources.md)
...
version of their guestbook application after every build in the tip of master
   * To address this use
   case, the application would expose a parameter named `image`, whose value used in the `dev`
   environment contains a placeholder value (e.g. `example/guestbook:replaceme`)
   * The placeholder value
   would be determined externally (outside of Git) such as a build system
   * Then, as part of the build
   pipeline, the parameter value of the `image` would be continually updated to the freshly built image
   (e.g. `argocd app set guestbook -p image=example/guestbook:abcd123`)
   * A sync operation
...
2. A repository of Helm manifests is already publicly available (e.g. https://github.com/helm/charts).
   * Since commit access to the repository is unavailable, it is useful to be able to install charts from
   ...
   forking the repository to make the changes
   * For example, to install Redis from the Helm chart
